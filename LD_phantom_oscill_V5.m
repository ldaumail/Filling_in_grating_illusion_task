...
exp.longDriftPos = [zeros(1,exp.initialFixation*exp.flipsPerSec)...
    repmat([exp.stillDotPhase exp.driftPos exp.driftPos(1:length(exp.driftPos)/2) zeros(1,(exp.blockLength-exp.trialFixation)*exp.flipsPerSec-1.5*length(exp.driftPos)) zeros(1,exp.betweenBlocks*exp.flipsPerSec)],1,exp.numBlocks-1)... %2*ones(1,e.blockLength) zeros(1,e.betweenBlocks)
    exp.stillDotPhase exp.driftPos exp.driftPos(1:length(exp.driftPos)/2) zeros(1,(exp.blockLength-exp.trialFixation)*exp.flipsPerSec-1.5*length(exp.driftPos)) zeros(1,exp.finalFixation*exp.flipsPerSec)];
