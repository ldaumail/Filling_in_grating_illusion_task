function LD_rivalry_percept_test_v6(subject, session, debug, cont)
...
ex.blockLength = ex.trialFixation + 59;%119; %ex.trialFixation+ ceil(ex.stimDur*ex.stimsPerBlock);           % in seconds
...
ex.lcstim.contrast = cont; %0.03;%linspace(0.01,0.20,10);%[0.05, 0.10, 0.15];                                                 % in %, maybe?? %here the number of stimulus contrast levels is the number of different conditions
...
ex.conds = {'NophLeft', 'NophLeft' ...%, 'MinbgRightVel3', 'MeanbgRightVel3', 'LightbgRightVel3', 'NophRight'
    ... %, Here, the Left/Right indicator in the condition name corresponds to the phantom grating pair location on the screen 
    }; %%'MinbgTopLeftVel3', 'MinbgBotLeftVel3',  'MeanbgTopLeftVel3', 'MeanbgBotLeftVel3', 'LightbgTopLeftVel3','LightbgBotLeftVel3', 'MinbgPairLeftVel3', 'MeanbgPairLeftVel3', 'LightbgPairLeftVel3',
...
phase = 0; %ex.stim.phases(c,1,1,1);
...
respDat = nan(50,length(responseType), ex.repsPerRun, ex.numConds); %response time data better organized
respFreq = nan(length(responseType), ex.repsPerRun, ex.numConds); %response frequency data better organized
...
totalFreq = sum(respFreq, 3, 'omitnan');

fprintf('Switch Frequency for Invisible percept = %f\n', totalFreq(1))
fprintf('Switch Frequency for Mixed percept = %f\n', totalFreq(2))
fprintf('Switch Frequency for Fully visible percept = %f\n', totalFreq(3))

respDuration = nan(50, length(responseType), ex.repsPerRun, ex.numConds); %response frequency data better organized
cumulRespDur = nan(length(responseType), ex.repsPerRun, ex.numConds); %response frequency data better organized
    ...
    for n = 1:length(resps)-1
        for t = 2:length(resps)
            if (resps(n) ~= resps(t)) & (n+1 == t)
                respT1 = respTimes(n);
                respT2 = respTimes(t);
                duration = respT2 - respT1;
                break
            end
        end
        respDuration(n,resps(n),cnt(condNum), condNum) = duration; %n = response number (only one response per row across all response types), resps(n) = response type,
        
    end
   
    cumulRespDur(:,cnt(condNum), condNum) = 100*sum(respDuration(:,:,cnt(condNum), condNum),1, 'omitnan')/120; %devide by 120 s since trial lasts 2 minutes
end
 avgRespDur = squeeze(nanmean(respDuration,1)); %(:,cnt(condNum), condNum)
 meanRespDur = mean(avgRespDur,2);
 
fprintf('Mean percept duration for Invisible percept = %f\n', meanRespDur(1))
fprintf('Mean percept duration for Mixed percept = %f\n', meanRespDur(2))
fprintf('Mean percept duration for Fully visible percept = %f\n', meanRespDur(3))

totalCumulRespDur = sum(cumulRespDur,3);
fprintf('Cumulative percept duration for Invisible percept = %f\n', totalCumulRespDur(1))
fprintf('Cumulative percept duration for Mixed percept = %f\n', totalCumulRespDur(2))
fprintf('Cumulative percept duration for Fully visible percept = %f\n', totalCumulRespDur(3))