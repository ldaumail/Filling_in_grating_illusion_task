function LD_phantom_single_vs_pair_percept(subject, session, debug)
...
ex.conds = {'MinbgPairVel3','MinbgTopVel3','MinbgBotVel3','MeanbgPairVel3','MeanbgTopVel3','MeanbgBotVel3'...%,
    ... %,'MeanbgRedDotVel2',
    }; 
...
ex.trialFlips = (0:ex.flipWin:ex.blockLength(1));
...
coTopaperture=Screen('OpenOffscreenwindow', w, gray1);
Screen('FillRect',coTopaperture, [255 255 255 0], [xc-(1/2)*ex.gaborWidth yc-(3/2)*ex.gaborHeight xc+ex.gaborWidth/2 yc-ex.gaborHeight/2]);

coBotaperture=Screen('OpenOffscreenwindow', w, gray1);
Screen('FillRect',coBotaperture, [255 255 255 0], [xc-(1/2)*ex.gaborWidth yc+ex.gaborHeight/2 xc+ex.gaborWidth/2 yc+(3/2)*ex.gaborHeight]);
...
phTopaperture=Screen('OpenOffscreenwindow', w, gray2);
Screen('FillRect',phTopaperture, [255 255 255 0], [xc-(1/2)*ex.gaborWidth yc-(3/2)*ex.gaborHeight xc+ex.gaborWidth/2 yc-ex.gaborHeight/2]);

phBotaperture=Screen('OpenOffscreenwindow', w, gray2);
Screen('FillRect',phBotaperture, [255 255 255 0], [xc-(1/2)*ex.gaborWidth yc+ex.gaborHeight/2 xc+ex.gaborWidth/2 yc+(3/2)*ex.gaborHeight]);


%% %%%% initial window - wait for backtick
DrawFormattedText(w,'Watch the oscillating grating or visual phantom within the gap at the center of the screen \n\n What do you see? \n\n Press Space to start'... % :  '...
    ,'center', 'center',[0 0 0]);
Screen(w, 'Flip', 0);
%WaitSecs(2);
    ...
    %flip through the block and following between block time
        ...
        if contains(condName, 'Minbg')
            Screen('FillRect', w, gray2);
                ...
                if contains(condName, 'Pair')
                    xOffset = ex.stimLongDriftPos(cMinP,1,n)-ex.stimLongDriftPos(cMinP,1,1); %baseline correct the position since every image already hase a spatial phase shift in the sinewave
                    ex.rectLRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.rawGaborHeight],xc+xOffset,yc);
                    % stim
                    Screen('DrawTexture', w, ex.rectSWaveID(cMinP,1),[],ex.rectLRect);
                    Screen('DrawTexture',w,phaperture);
                elseif contains(condName, 'Top')
                    xOffset = ex.stimLongDriftPos(cMinT,1,n)-ex.stimLongDriftPos(cMinT,1,1); %baseline correct the position since every image already hase a spatial phase shift in the sinewave
                    ex.rectLRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.rawGaborHeight],xc+xOffset,yc);
                    % stim
                    Screen('DrawTexture', w, ex.rectSWaveID(cMinT,1),[],ex.rectLRect);
                    Screen('DrawTexture',w,phTopaperture);
                elseif contains(condName, 'Bot')

                    xOffset = ex.stimLongDriftPos(cMinB,1,n)-ex.stimLongDriftPos(cMinB,1,1); %baseline correct the position since every image already hase a spatial phase shift in the sinewave
                    ex.rectLRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.rawGaborHeight],xc+xOffset,yc);
                    % stim
                    Screen('DrawTexture', w, ex.rectSWaveID(cMinB,1),[],ex.rectLRect);
                    Screen('DrawTexture',w,phBotaperture);
                end
        ...
        elseif contains(condName, 'Meanbg')
            Screen('FillRect', w, gray1);
                ...
                if contains(condName, 'Pair')
                    xOffset = ex.stimLongDriftPos(cMeanP,1,n)-ex.stimLongDriftPos(cMeanP,1,1); %baseline correct the position since every image already hase a spatial phase shift in the sinewave
                    ex.rectLRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.rawGaborHeight],xc+xOffset,yc);
                    % stim
                    Screen('DrawTexture', w, ex.rectSWaveID(cMeanP,1),[],ex.rectLRect);
                    Screen('DrawTexture',w,coaperture);
                elseif contains(condName, 'Top')
                    xOffset = ex.stimLongDriftPos(cMeanT,1,n)-ex.stimLongDriftPos(cMeanT,1,1); %baseline correct the position since every image already hase a spatial phase shift in the sinewave
                    ex.rectLRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.rawGaborHeight],xc+xOffset,yc);
                    % stim
                    Screen('DrawTexture', w, ex.rectSWaveID(cMeanT,1),[],ex.rectLRect);
                    Screen('DrawTexture',w,coTopaperture);
                elseif contains(condName, 'Bot')
                    xOffset = ex.stimLongDriftPos(cMeanB,1,n)-ex.stimLongDriftPos(cMeanB,1,1); %baseline correct the position since every image already hase a spatial phase shift in the sinewave
                    ex.rectLRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.rawGaborHeight],xc+xOffset,yc);
                    % stim
                    Screen('DrawTexture', w, ex.rectSWaveID(cMeanB,1),[],ex.rectLRect);
                    Screen('DrawTexture',w,coBotaperture);
                end
