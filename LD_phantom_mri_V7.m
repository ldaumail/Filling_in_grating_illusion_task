...
if debug == 1
	...
	ex.resolution = SetResolution(max(Screen('Screens')),1024,768,60); % laptop 1920,1080
...
while(1)
    ...
    if mod(n, ex.flipsPerTrial) <= ex.trialOnFlips && strcmp(fixCol{:}, 'red')
        ...
        Screen('FillOval', w,[255 255 255], [xc-round(ex.bigFixSize/2) yc-round(ex.bigFixSize/2) xc+round(ex.bigFixSize/2) yc+round(ex.bigFixSize/2)]); % white fixation ring
        % little fixation dot
        Screen('FillOval', w,[150 0 0], [xc-round(ex.fixSize/2) yc-round(ex.fixSize/2) xc+round(ex.fixSize/2) yc+round(ex.fixSize/2)]); % black fixation dot
        
    elseif (mod(n, ex.flipsPerTrial) <= ex.flipsPerTrial && strcmp(fixCol{:}, 'black')) || (mod(n, ex.flipsPerTrial) > ex.trialOnFlips && strcmp(fixCol{:}, 'red')) % also accounts for after the target flashed
        %big fixation circle
        Screen('FillOval', w,[0 0 0], [xc-round(ex.bigFixSize/2+ex.outerFixPixels ) yc-round(ex.bigFixSize/2+ex.outerFixPixels ) xc+round(ex.bigFixSize/2+ex.outerFixPixels ) yc+round(ex.bigFixSize/2+ex.outerFixPixels )]); % black fixation ring
        Screen('FillOval', w,[255 255 255], [xc-round(ex.bigFixSize/2) yc-round(ex.bigFixSize/2) xc+round(ex.bigFixSize/2) yc+round(ex.bigFixSize/2)]); % white fixation ring
        % little fixation dot
