function LD_dynamic_MAE_v1(subject, session, debug, vertOffsets, horiOffsets)
...
ex.lcstim.luminanceRange = 0.2; %0.2; %0.03;%linspace(0.01,0.20,10);%[0.05, 0.10, 0.15];                                                 % in %, maybe?? %here the number of stimulus contrast levels is the number of different conditions
...
ex.test.cycles = 1; %number of grating cycles per lap
ex.test.cycPerSec = 0.25;
...
%     for l =1:length(ex.stim.contrastOffset)
%         for r = 1:max(ex.repsPerRun)
ex.test.tempPhase1 = pi/2;% rand(1,1)*2*pi;
ex.test.tempPhase2 = 3*pi/2;%pi;% rand(1,1)*2*pi;
ex.test.oscillation1 = cos(2*pi*(1/ex.testDur(1))*ex.test.flipTimes+ex.test.tempPhase1);
ex.test.oscillation2 = cos(2*pi*(1/ex.testDur(1))*ex.test.flipTimes+ex.test.tempPhase2);
ex.test.spatialPhase1 = 0;
ex.test.spatialPhase2 = 0;
...
ex.test.phases2 = ex.test.oscillation2.*180*ex.test.cycles(1)+ex.test.spatialPhase2;
%         end
%     end
% end
...
phases1 = ex.test.phases1;
phases2 = ex.test.phases2;
ex.testStim = nan(ex.rawProbeHeight,ex.rawProbeWidth,ex.testLength*ex.flipsPerSec);
for n = 1:ex.testLength*ex.flipsPerSec
    ex.testStim(:,:,n) = (makeSineGrating(ex.rawProbeHeight,ex.rawProbeWidth,ex.stim.spatialFreqDeg,...
        ex.stim.orientation,phases1(n),ex.lcstim.contrastOffset,ex.lcstim.contrastMultiplicator,...
        ex.ppd)+makeSineGrating(ex.rawProbeHeight,ex.rawProbeWidth,ex.stim.spatialFreqDeg,...
        ex.stim.orientation,phases2(n),ex.lcstim.contrastOffset,ex.lcstim.contrastMultiplicator,...
        ex.ppd))./2;
