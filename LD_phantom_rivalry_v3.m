function LD_phantom_rivalry_v3(subject, session, debug)
...
ex.blockLength = ex.trialFixation + 5;%119; %ex.trialFixation+ ceil(ex.stimDur*ex.stimsPerBlock);           % in seconds
...
ex.lcstim.contrast = 0.020;%linspace(0.01,0.20,10);%[0.05, 0.10, 0.15];                                                 % in %, maybe?? %here the number of stimulus contrast levels is the number of different conditions
...
ex.conds = {'MinbgLeftVel3','MinbgRightVel3','MeanbgLeftVel3', 'MeanbgRightVel3', 'NophLeft', 'NophRight' ...%,
...
ex.stim.tempPhase1 = nan(2,length(ex.stim.contrastOffset), ex.repsPerRun);
ex.stim.tempPhase2 = nan(2,length(ex.stim.contrastOffset), ex.repsPerRun);
ex.stim.oscillation1 = nan(2,length(ex.stim.contrastOffset), ex.repsPerRun,length(ex.stim.flipTimes));
ex.stim.oscillation2 = nan(2,length(ex.stim.contrastOffset), ex.repsPerRun,length(ex.stim.flipTimes));
ex.stim.phases = nan(2,length(ex.stim.contrastOffset), ex.repsPerRun,length(ex.stim.flipTimes));
...
ex.rectSWave = nan(2, length(ex.stim.contrastOffset), ex.repsPerRun,ex.rawGaborHeight,ex.rawGaborWidth);
ex.rectSWaveID = nan(2, length(ex.stim.contrastOffset), ex.repsPerRun);
clear c r
for c =1:2
    for l = 1:length(ex.stim.contrastOffset)
...
ex.stimDriftPosDeg = nan(2,length(ex.stim.contrastOffset), ex.repsPerRun,length(ex.stim.oscillation1(1,1,1,:)));
ex.stimDriftPos = nan(2,length(ex.stim.contrastOffset),ex.repsPerRun,length(ex.stim.oscillation1(1,1,1,:)));
ex.stimLongDriftPos = nan(2,length(ex.stim.contrastOffset), ex.repsPerRun,length(flipTimes));
clear c r
for c = 1:2
    for l =1:length(ex.stim.contrastOffset)
        for r = 1:ex.repsPerRun
...
Screen('FillRect',coLaperture, [255 255 255 0], [xc-(1/2)*(ex.probeWidth - (xc/2)) yc-ex.probeHeight/2 xc+(ex.probeWidth+(xc/2))/2 yc+(1/2)*ex.probeHeight]); %opposite eye grating window
...
Screen('FillRect',coRaperture, [255 255 255 0], [xc-(1/2)*(ex.probeWidth + (xc/2)) yc-ex.probeHeight/2 xc+(ex.probeWidth-(xc/2))/2 yc+(1/2)*ex.probeHeight]); %opposite eye grating window
...
Screen('FillRect',phLaperture, [255 255 255 0], [xc-(1/2)*(ex.probeWidth - (xc/2)) yc-ex.probeHeight/2 xc+(ex.probeWidth+(xc/2))/2 yc+(1/2)*ex.probeHeight]); %opposite eye grating window
...
Screen('FillRect',phRaperture, [255 255 255 0], [xc-(1/2)*(ex.probeWidth + (xc/2)) yc-ex.probeHeight/2 xc+(ex.probeWidth-(xc/2))/2 yc+(1/2)*ex.probeHeight]); %opposite eye grating window
        ...
        if contains(condName, 'Minbg')
            ...
            if nnz(find(ex.longFormStimOnSecs(n)))
                if contains(condName, 'Left')
                    ...
                    ex.lcLRect =  CenterRectOnPoint([0 0 ex.rawProbeWidth ex.rawProbeHeight],xc+xc/4,yc);
                    % stim
                ...
                elseif contains(condName, 'Right')
                    ...
                    ex.rectLRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.rawGaborHeight],xc+xc/2+xOffset,yc);
                    ex.lcRRect =  CenterRectOnPoint([0 0 ex.rawProbeWidth ex.rawProbeHeight],xc-xc/4,yc);
                    % stim
