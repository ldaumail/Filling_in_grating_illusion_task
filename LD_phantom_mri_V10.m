function LD_phantom_mri_V10(subject, session, horiOffset, vertOffset, debug, figSizeDeg) 
...
if debug == 1
    ...
    ex.resolution = SetResolution(max(Screen('Screens')),1024,768, 60); % scanner 1024,768, 60 1024,768, 60
...
ex.vertOffset = vertOffset;    % vertical offset from findOffset_phantom.m
ex.horiOffset = horiOffset;    % horizontal offset from findOffset_phantom.m
...
xc = rect(3)/2+ex.horiOffset; % rect and center, with the flexibility to resize & shift center - change vars to zero if not used.
yc = rect(4)/2+ex.vertOffset+ex.yFixOffset; %+e.vertOffset;

xL = rect(3)/2- ex.gaborWidth+ex.horiOffset;  % = stimulus center located on the horizontal center of the screen
yL = rect(4)/2+ex.vertOffset +ex.yStimOffset; % stimulus located 4 degrees above screen center

xR = rect(3)/2+ ex.gaborWidth+ex.horiOffset; % = stimulus center located on the horizontal center of the screen
yR = rect(4)/2+ex.vertOffset+ex.yStimOffset; % stimulus located 4 degrees below screen center
