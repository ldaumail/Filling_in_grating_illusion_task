function [vertOffsets,horiOffsets] = findOffset_nonius_fixationV5(subject, session, debug)
...
responseKeys(KbName('UpArrow'))=1; % button box 
responseKeys(KbName('DownArrow'))=1; % button box 
...
text = 'Press 1 (left square) or 2 (right square) \n\n to select a square to move.';   
DrawFormattedText(w, text, xc/5, yc/2);
DrawFormattedText(w, text, xc+xc/5, yc/2);
Screen(w, 'Flip', 0);
%%%%%%%%
...
while 1
    ...
    if keyIsDown
        ...
        elseif keyCode(KbName('2'))%keyCode(KbName('RightArrow'))
            chosenSquare = 'Right';
...
%     %% square
    ...
    if contains(chosenSquare, 'Right')
        if find(firstPress,1) == KbName('UpArrow')
            ex.vertOffsetR = ex.vertOffsetR - 10;
            KbQueueFlush();
        elseif find(firstPress,1) == KbName('DownArrow')
            ex.vertOffsetR = ex.vertOffsetR + 10;
            KbQueueFlush();
        elseif find(firstPress,1) == KbName('LeftArrow')
            ex.horiOffsetR = ex.horiOffsetR - 10;
            KbQueueFlush();
        elseif find(firstPress,1) == KbName('RightArrow')
            ex.horiOffsetR = ex.horiOffsetR + 10;
    ...
    elseif contains(chosenSquare, 'Left')
        if find(firstPress,1) == KbName('UpArrow')
            ex.vertOffsetL = ex.vertOffsetL - 10;
            KbQueueFlush();
        elseif find(firstPress,1) == KbName('DownArrow')
            ex.vertOffsetL = ex.vertOffsetL + 10;
            KbQueueFlush();
        elseif find(firstPress,1) == KbName('LeftArrow')
            ex.horiOffsetL = ex.horiOffsetL - 10;
            KbQueueFlush();
        elseif find(firstPress,1) == KbName('RightArrow')
            ex.horiOffsetL = ex.horiOffsetL + 10;
            KbQueueFlush();
