function LD_phantom_oscill_v16_ETcomp(subject, session, debug)
...
for c =1:length(ex.condShuffle)
    ...
    while n <= length(ex.trialFlips)
        ...
        if n == 1
            ...
            [VBLT,ex.flipTime(n,c), FlipT, missed] = Screen(w, 'Flip',ex.startTrial + ex.trialFlips(n) - slack); %,   %%% ex.flipTime(n,c)
