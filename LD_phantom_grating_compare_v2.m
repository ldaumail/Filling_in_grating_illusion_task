...
ex.test.backgroundLum = [gray1; gray2; gray3];
...
ex.test.contrastOffset = [(ex.test.backgroundLum(1,1)./255)./(1+ex.test.contrast),(ex.test.backgroundLum(2,1)./255)./(1+ex.test.contrast), (ex.test.backgroundLum(3,1)./255)./(1+ex.test.contrast)]; %(ex.stim.backgroundLum(2,1)./255)./(1+ex.stim.contrast), ex.stim.backgroundLum(3,1)./255];%+ex.stim.contrast/2;
...
ex.conds = {'MinbgPairOn','MaxbgPairOn','MeanbgPairDark'}; 
...
ex.stim.backgroundLum = [gray1;gray2;gray3];
...
while(1) %n <= length(ex.trialFlips)
    ...
    Screen('FillRect', w, ex.stim.backgroundLum(thisCond,:))
