function LD_phantom_single_vs_pair_v1(subject, session, debug)
...
cntMinP = 0; %count number of summed gratings condition trials, as the phase is random, we need to have a different phase at each new trial
cntMinT = 0;
cntMinB = 0;
cntMeanP = 0;
cntMeanT = 0;
    ...
    while n <= length(ex.trialFlips)
        ...
        if contains(condName, 'Minbg')
            ...
            if nnz(find(ex.longFormStimOnSecs(n)))
                if contains(condName, 'Pair')
                    xOffset = ex.stimLongDriftPos(cMinP,cntMinP,n)-ex.stimLongDriftPos(cMinP,cntMinP,1); %baseline correct the position since every image already hase a spatial phase shift in the sinewave
                    ex.rectLRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.rawGaborHeight],xc+xOffset,yc);
                    % stim
                    Screen('DrawTexture', w, ex.rectSWaveID(cMinP,cntMinP),[],ex.rectLRect);
                    Screen('DrawTexture',w,phaperture);
                elseif contains(condName, 'Top')
                    xOffset = ex.stimLongDriftPos(cMinT,cntMinT,n)-ex.stimLongDriftPos(cMinT,cntMinT,1); %baseline correct the position since every image already hase a spatial phase shift in the sinewave
                    ex.rectLRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.rawGaborHeight],xc+xOffset,yc);
                    % stim
                    Screen('DrawTexture', w, ex.rectSWaveID(cMinT,cntMinT),[],ex.rectLRect);
                    Screen('DrawTexture',w,phTopaperture);
                elseif contains(condName, 'Bot')

                    xOffset = ex.stimLongDriftPos(cMinB,cntMinB,n)-ex.stimLongDriftPos(cMinB,cntMinB,1); %baseline correct the position since every image already hase a spatial phase shift in the sinewave
                    ex.rectLRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.rawGaborHeight],xc+xOffset,yc);
                    % stim
                    Screen('DrawTexture', w, ex.rectSWaveID(cMinB,cntMinB),[],ex.rectLRect);
        ...
        elseif contains(condName, 'Meanbg')
            ...
            if nnz(find(ex.longFormStimOnSecs(n)))
                ...
                if contains(condName, 'Pair')
                    ...
                    ex.rectLRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.rawGaborHeight],xc+xOffset,yc);
                    % stim
                    Screen('DrawTexture', w, ex.rectSWaveID(cMeanP,cntMeanP),[],ex.rectLRect);
                    Screen('DrawTexture',w,coaperture);
                elseif contains(condName, 'Top')
                    xOffset = ex.stimLongDriftPos(cMeanT,cntMeanT,n)-ex.stimLongDriftPos(cMeanT,cntMeanT,1); %baseline correct the position since every image already hase a spatial phase shift in the sinewave
                    ex.rectLRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.rawGaborHeight],xc+xOffset,yc);
                    % stim
                    Screen('DrawTexture', w, ex.rectSWaveID(cMeanT,cntMeanT),[],ex.rectLRect);
                    Screen('DrawTexture',w,coTopaperture);
                elseif contains(condName, 'Bot')
                    xOffset = ex.stimLongDriftPos(cMeanB,cntMeanB,n)-ex.stimLongDriftPos(cMeanB,cntMeanB,1); %baseline correct the position since every image already hase a spatial phase shift in the sinewave
                    ex.rectLRect =  CenterRectOnPoint([0 0 ex.rawGaborWidth ex.rawGaborHeight],xc+xOffset,yc);
                    % stim
                    Screen('DrawTexture', w, ex.rectSWaveID(cMeanB,cntMeanB),[],ex.rectLRect);
