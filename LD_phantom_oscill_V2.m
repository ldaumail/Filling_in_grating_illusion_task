...
exp.stim.cycles = 1; %number of cycles shifted per lap  (modified to half the number of cycles per lap)
...
exp.stim.phases = cos(2*pi*(1/exp.stimDur)*flipTimes).*180*exp.stim.cycles; %./exp.stimDur-2*pi*flipTimes./exp.stimDur make it oscillatory
...
%5)Then we subtract the phase at each flip time t which here is 0.
