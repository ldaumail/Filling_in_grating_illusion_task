function LD_localizer_horiOffset(subject, session, horiOffset, vertOffset, locSize, debug) 
% LD. 01272022
...
loc.horiOffset = horiOffset;    % vertical offset from FindScreenSize.m
...
loc.yStimOffset = -1*loc.ppd;
loc.yFixOffset = 3*loc.ppd;

xc = rect(3)/2+loc.horiOffset; % rect and center, with the flixibility to resize & shift center - change vars to zero if not used.
yc = rect(4)/2+loc.vertOffset+loc.yFixOffset;

xS = rect(3)/2+loc.horiOffset;  % = stimulus center located on the horizontal center of the screen
yS = rect(4)/2+loc.vertOffset +loc.yStimOffset; % stimulus located 4 degrees above screen center
...
checkRect = CenterRectOnPoint([0 0 size(checkerboard,1) size(checkerboard,2)],xS,yS);
...
Screen('FillRect',aperture2, [255 255 255 0], [xS-(1/2)*loc.stimSize yS-loc.stimSize/2 xS+loc.stimSize/2 yS+loc.stimSize/2]);

aperture=Screen('OpenOffscreenwindow', win, 128);
Screen('FillRect',aperture, [255 255 255 0], [xS-(3/2)*loc.stimSize yS-loc.stimSize/2 xS-loc.stimSize/2 yS+loc.stimSize/2]);
Screen('FillRect',aperture, [255 255 255 0], [xS+(1/2)*loc.stimSize yS-loc.stimSize/2 xS+loc.stimSize*(3/2) yS+loc.stimSize/2]);
